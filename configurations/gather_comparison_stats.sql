...
with agg_data as (SELECT test_type,
       ...
       max(total_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following) max_total_query_time_seconds,
       mean_query_time_seconds,
       max(mean_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following ) max_mean_query_time_seconds,
       median_query_time_seconds,
       max(median_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following ) max_median_query_time_seconds,
       max_query_time_seconds,
       max(max_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following ) max_max_query_time_seconds,
       pct75_query_time_seconds,
       max(pct75_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following ) max_pct75_query_time_seconds,
       pct90_query_time_seconds,
       max(pct90_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following ) max_pct90_query_time_seconds,
       pct95_query_time_seconds,
       max(pct95_query_time_seconds) over (partition by test_type rows between unbounded preceding and unbounded following ) max_pct95_query_time_seconds,
       count_cc_scaling,
       count_aborted,
       count_queries,
       analysis_timestamp
